...
class CountdownBot extends SlackWrapper {
	...
	onSignupSuccess (botAccessToken, response) {
	    ...
	    if (channel) {
	      this._sayHello({channel_uid: channel}, {token: botAccessToken});
	    } else {
	...
	_sayHello (event, settings) {
		this.postMessage(event.event.channel, presets.sayHello, settings);
	}
	...
	_randomRepliesToChannel(event, settings) {
		...
		if (this.isSillyType(dogSounds, event)) {
			this.postMessage(event.event.channel, utils.pickRandom(dogSounds) + '!', settings);
			...
			if (command.length > 0) {
				...
				`Dangit, I never learned how to ${command[0]}. I only know how to count, read, write, and bark, embarrassingly enough.`, 
				settings);
			}
	...
	_generateResponse (event, settings) {
		return Countdown.findOne({teamId: settings.teamId, event: settings.countdown}).then(function(countdown) {
			...
			if (days <= 0) {
				this._deleteCountdown({event: countdownEvent}, event, true);
