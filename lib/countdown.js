...
class CountdownBot extends Slackbot {
	...
	handleNewChronJob (countdown, message) {
		...
		if (ruleIsInOldFormat) {
			...
			console.log('handling new chron job', countdown.event);
			...
			console.log('scheduling', countdown.event);
				...
				if (days === 0) {
					this._deleteCountdown({event: event}, message, true);
					reminder =`Hooray! Today is the day for ${event}! You lucky dowgs, you.`
				} else if (days < 0) {
					this._deleteCountdown({event: event}, message, true);
					reminder =`Looks like ${event} already happened! I just went ahead and removed that countdown for ya. Arf!`
				} else {
					reminder = `And now for your scheduled reminder: ${days} ${days > 1 ? 'days' : 'day'} until ${countdown.event}!`;
				}
