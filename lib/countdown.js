...
class CountdownBot extends Slackbot {
	...
	_sayHello(message) {
		this.postMessage(message.channel, presets.sayHello, {as_user: this.user.name});
	}
	...
	handleNewChronJob (countdown) {
		...
		var scheduled = nodeSchedule.scheduleJob(countdown.schedule.rule, function(){
			var message = `And now for your scheduled reminder: ${moment(countdown.date).diff(new Date(), 'days')} until ${countdown.event}!`;
