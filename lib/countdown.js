...
class CountdownBot extends Slackbot {
	...
	_updateCountdown (query, updatedCountdown, message,
		...
		console.log('about to update', query, updatedCountdown.event)
		    ...
		    function (err, modifiedCountdown) {
		      	if (!!err) {
					...
					this.postMessage(message.channel,
		      		...
		      		console.log('it worked?', modifiedCountdown.event, modifiedCountdown.schedule.rule);
		      		if (successCb) {
	...
	handleNewChronJob (countdown, message) {
		...
		if (ruleIsInOldFormat) {
			...
			var today = moment();
			var days = Math.ceil(moment(countdown.date).diff(today, 'hours') / 24)
			var scheduled = nodeSchedule.scheduleJob(rule, function(){
				var reminder = `And now for your scheduled reminder: ${days} ${days > 1 ? 'days' : 'day'} until ${countdown.event}!`;
