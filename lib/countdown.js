...
class CountdownBot extends SlackWrapper {
	...
	deleteBot (teamId) {
		Bot.deleteOne({'teamId': teamId})
		.then(deleted => console.log(`deleted ${deleted} for ${teamId}`))
		.catch(err => console.error(`Couldn't delete bot for ${teamId}`));
	}
	...
	_randomRepliesToChannel(event, settings) {
		...
		if (this.isSillyType(dogSounds, event)) {
			...
			let command = _.filter(dogCommands, phrase => phrase.test(event.event.text.toLowerCase()));
	...
	_generateResponse (event, settings) {
		return Countdown.findOne({teamId: settings.teamId, event: settings.countdown}).then(function(countdown) {
			...
			if (days === 0) {
        console.log(`DELETING ${countdownEvent}, settings: ${settings}`);
				...
				this._deleteCountdown({...settings, event: countdownEvent}, {}, true);
				return `Welp, looks like ${countdownEvent} already happened! I'll remove that event for you.`
			}
		...
		Countdown.findOne({teamId: newSettings.teamId, event: newSettings.event})
			.then(function(countdown) {
				if (!!countdown) {
					...
					this._saveCountdown(newSettings, event);
				}
			}.bind(this));
	}
	...
	_deleteCountdown (settings, event, skipMessage) {
		...
		.then(deleted => {
	    	...
	    	if (!skipMessage && !_.isEmpty(event)) {
				this.postMessage(event.event.channel,
  ...
  isToday(eventDate) {
    const rightNow = moment().tz('America/Los_Angeles');
    const h = moment(eventDate).tz('America/Los_Angeles');
    return h.date() === rightNow.date() && h.year() === rightNow.year() && h.month() === rightNow.month();
  }

  alreadyHappened(eventDate) {
    const rightNow = moment().tz('America/Los_Angeles');
    const h = moment(eventDate).tz('America/Los_Angeles');
    return h.isBefore(rightNow);
  }

	executeReminder (countdown) {
		const { event, date, teamId } = countdown;
		
		var reminder;
		var today = moment().tz("America/Los_Angeles");
		var hours = moment(date).diff(today, 'hours');
		var rawDays = hours/24;
		var days = Math.ceil(rawDays);

		var settings = { teamId, event, token: countdown.botAccessToken };
		...
		} else if (this.alreadyHappened(date)) {
			this._deleteCountdown(settings, {}, true);
