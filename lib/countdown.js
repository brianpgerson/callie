...
class CountdownBot extends SlackWrapper {
	...
	onMessage (message) {
	    if (utils.isValidMessage(message, this.user.id)) {
        	...
        	switch (type) {
        		case 'help':
        			let settings = utils.extractSettings(message, 'idOnly');
        			this._sendHelpResponse(message, settings);
        			break;
        		case 'start':
        			settings = utils.extractSettings(message, 'date');
        			if (settings.dumb) {
        		...
        		case 'reset':
        			...
        			break;
        		case 'schedule':
        			settings = utils.extractSettings(message, 'schedule');
        			this._reformatChron(settings, message);
        			break;
        		case 'cancel':
        			settings = utils.extractSettings(message, 'event');
        		...
        		case 'delete':
        			...
        			break;
        		case 'list':
        			settings = utils.extractSettings(message, 'idOnly');
        			this._listTeamEvents(message, settings);
        			break;
        		case 'countdown':
        			settings = utils.extractSettings(message, 'countdown');
	...
	_noDumbPlease (message, settings) {
		this.postMessage(message.channel, presets.nodumb, settings);
	}

	_sayHello (message, settings) {
		console.log(message);
		this.postMessage(message.channel, presets.sayHello, settings);
	}
	...
	_listTeamEvents (message, settings) {
		Countdown.find({teamId: message.team}).then(function (countdowns) {
			...
			this.postMessage(message.channel, reply, settings);
		}.bind(this));
	}

	_sendHelpResponse (message, settings) {
		var helpMessage = `Happy to help! ${presets.help}`;
		this.postMessage(message.channel, helpMessage, settings);
	}
	...
	handleNewChronJob (countdown, message) {
		...
		if (ruleIsInOldFormat) {
			...
			console.log('handling new chron job', countdown.event, message.team);
			...
			var scheduled = nodeSchedule.scheduleJob(rule, function(){
				...
				_this.postMessage(message.channel, reminder, {as_user: 'callie'});
			});
