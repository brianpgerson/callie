...
To update, simply say:

\`@thecount update destination: <your destination>, event: <your event name>, date: <the date, formatted YYYY-MM-DD>\`
...
See ya!`;
	...
	_extractType (message) {
		...
		var update = new RegExp(/update/);
		if (messageText.match(help)) {
			return 'help';
		} else if (messageText.match(update)) {
			return 'update';
		}
	}
	...
	_extractUpdateSettings (message) {
		...
		var cleanStringSettings = originalMessage.slice(text.indexOf('destination')).split(',');
	...
	_generateResponse () {
		var builtMessage = `${this._pickRandom(this.openers)} ${this.date.diff(new Date(), 'days')} remaining`;
