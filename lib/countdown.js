...
class CountdownBot extends Slackbot {
	...
	updateSavedRule (rule, countdown, message) {
		...
		var hour = parseInt(rule[2]);
		var day = parseInt(rule[5][0]);
		newRule.hour = hour;
		if (_.isFinite(day)) {
			newRule.dayOfWeek = day;
		}
		console.log('updating countdown rule:', countdown.event, newRule);
	...
	_updateChron (settings, message, rule) {
		...
		this._updateCountdown(
			...
			function (updatedCountdown) {
				console.log('it worked?', updatedCountdown.event, updatedCountdown.schedule.rule);
