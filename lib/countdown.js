...
class CountdownBot extends Slackbot {
	...
	_resetCountdown(newSettings, message) {
		...
		_.assign(newSettings, {schedule: {rule: '0 0 10 * * 1', active: true}});
	...
	_reformatChron (settings, message) {
		...
		chron[2] = !!hour ? utils.handleHour(hour, chron[2], message, this) : chron[2];
		if (settings.schedule === 'weekly') {
			if (!!day) {
				chron[5] = utils.handleDay(day, chron[5], message, this);
			}
	...
	_updateChron (settings, message, chron) {
		...
		this._updateCountdown(
			{teamId: message.team, event: settings.event},
			{schedule: {rule: rule, active: true, channel: message.channel, id: scheduleId}},
			message,
